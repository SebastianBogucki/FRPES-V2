...
//Simulation values
float S_newtime;
float S_Lastaltitude;
float S_Lasttime;
float S_deltaTime;
float S_Deltaaltitude;
float S_Apogee;
float S_ApogeeTime;
bool S_ApogeeRecorded = false;
...
void loop() {
  if(simulationMode == false && debugMode == false){
    Measurements();    
  }
  else{
    MissionSimulation();
...
void MissionSimulation(){
  sensors_event_t accel;
  sensors_event_t gyro;
  sensors_event_t temp;
  dso32.getEvent(&accel, &gyro, &temp);
    
  S_Lasttime = (float)millis()/1000;
  S_deltaTime = (float)S_Lasttime-(float)S_newtime;
  S_newtime = (float)millis()/1000;
  S_Deltaaltitude = S_Lastaltitude-final_Altitude;
  S_Lastaltitude = final_Altitude;
  roll = gyro.gyro.x;
  ...
  if(S_newtime>0 && S_newtime<4){
    Serial.print("deltatime: ");
    Serial.println(S_newtime);
    Acceleration = 109;
    final_Altitude = (Acceleration*pow(S_newtime, 2))/2;
    Velocity + Acceleration*S_deltaTime;
    roll = gyro.gyro.x;
    tiltY = accel.acceleration.y;
    tiltZ = accel.acceleration.z; 
    while (gpsSerial.available())
  ...
  else if(S_newtime>4 && final_Altitude > 1){
    if(S_ApogeeRecorded == false){
      S_Apogee = final_Altitude;      
      S_ApogeeTime = S_newtime;
      S_ApogeeRecorded = true;
    }
    Acceleration = -9.81;  
    final_Altitude = S_Apogee + (Acceleration*pow(S_newtime - S_ApogeeTime, 2))/2;
    Velocity + Acceleration*S_deltaTime;;
    roll = gyro.gyro.x;
    tiltY = accel.acceleration.y;
    tiltZ = accel.acceleration.z; 
...
void Controller(){
  switch(mode){
    ...
    //Mode 3
    case 3:
      if(Velocity<=0.5 && Velocity>=-0.5 && final_Altitude<30){
        if(landed == false){
